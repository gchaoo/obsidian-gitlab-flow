...
- 目标评论链接会按插件设置 `评论目标字段名` 逐个从 frontmatter 中读取，多个字段用英文逗号分隔，默认只读取 `相关链接`
- 目标链接必须是 GitLab Issue 评论链接，格式需包含 `#note_xxx`；如果只有 Issue 链接而没有评论 id，会直接报错
- 该按钮只会更新已有评论，不会新建评论
- 同步前会从 frontmatter 中移除忽略字段，当前默认忽略 `实时记录`
- 文档中的本地图片会先上传到目标 GitLab 项目，再将正文中的图片链接替换为 GitLab 上传地址；远程图片会保持原样
- 同步成功后，只更新 GitLab 评论内容，不会回写当前 Obsidian 文档
