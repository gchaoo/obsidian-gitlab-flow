...
- 当前文档文件名若不以 `YYYY-MM-DD` 开头，则发布前会按 `开始日期 + 空格 + 原文件名` 自动重命名
- `任务名称` 为空时，发布后会自动回填为当前文档文件名
- `开始日期`、`结束日期` 必填，且要求 `YYYY-MM-DD` 格式
- Issue 标题使用 `【相关合同】【相关软件】文章名称_yyyymmdd`，其中 `文章名称` 会忽略文件名开头的日期前缀
...
- 若表中存在 `PLM任务名称` 列，则按每一行生成并更新 `【合同名称】【软件名称】任务名称_任务类型_yyyymmdd`
- 保留其余列内容
- 将每一行的 `时间范围` 更新为 `开始日期～结束日期`

## 线上记录整理

`整理线上记录` 会读取当前笔记正文中 `## 千问记录` 标题下的第一个千问链接。
...
执行后会把页面里已有的摘要内容整理成固定区块，追加到正文末尾，并在重复执行时替换上一次生成的区块。若 `## 千问记录` 区块不存在，或区块下没有可用链接，则会直接报错。
