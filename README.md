...
- 若目标链接是 Issue / Work Item 链接，则更新对应正文，支持 `/-/issues/数字` 和 `/-/work_items/数字`
- 若目标链接是 Issue / Work Item 评论链接，格式需包含 `#note_xxx`，则更新对应评论
- 同步前会从 frontmatter 中移除忽略字段，当前默认忽略 `实时记录`
- 文档中的 Obsidian 文件片段嵌入 `![[文件#标题]]` / `![[文件#^blockId]]` 会展开为被引用 Markdown 片段原文；普通 wiki 链接 `[[文件#标题]]` 不展开；片段中的相对图片路径会按被引用文档所在目录解析后再上传
...
- 按每一行生成并更新 `【合同名称】【软件名称】任务名称_yyyymmdd`
- 保留其余列内容
